...
m <- multinom(formula = shelfFO ~ sugar + fat + sodium, data = norm.d)
summary(m)
...
$$\begin{aligned}
log(\frac{P(\text{shelf} = 3)}{P(\text{shelf} = 2)} = 14.78 - 14.91(\text{sugar}) - 4.62(\text{fat}) - 7.49(\text{sodium}) + \varepsilon
\end{aligned}$$
...
$$\begin{aligned}
log(\frac{P(\text{shelf} = 1)}{P(\text{shelf} = 2)} = -6.90 - 2.69(\text{sugar}) - 4.06(\text{fat}) + 17.49(\text{sodium}) + \varepsilon
\end{aligned}$$
...
$$\begin{aligned}
log(\frac{P(\text{shelf} = 4)}{P(\text{shelf} = 1)} = 14.39 - 14.09(\text{sugar}) - 4.93(\text{fat}) - 7.19(\text{sodium}) + \varepsilon
\end{aligned}$$
...
Based on the likelihood ratio test, sugar and sodium are important explanatory variables in determining where the cereal will be placed on the shelf.

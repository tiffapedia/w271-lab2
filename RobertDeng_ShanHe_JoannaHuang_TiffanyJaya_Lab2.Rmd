...
We chose the multinomial regression model to answer this question since the dependent variable, the probability of the cereal being placed on the shelf, has more than two categories. What we found is that cereal with high sugar content is placed on the second shelf (from the bottom) while cereal with low sugar content is placed on teh third shelf (from the bottom).  
...
**Question A: The explanatory variables need to be re-formatted before proceeding further. First, divide each explanatory variable by its serving size to account for the different serving sizes among the cereals. Second, re-scale each variable to be Symptoms Seizures Cardiac deficiency within 0 and 1.**
...
library(psych)
pairs.panels(norm.d[,c("sugar", "fat", "sodium")])
```
...
```{r}
m0 <- multinom(formula=shelf ~1, data=norm.d)
m2 <- multinom(formula = shelf ~ sugar*fat*sodium, data=norm.d)
#summary(cereal_m1)
Anova(m2)
...
# method 1: manually provide values for sugar

sugar <- seq(from=0, to=1, by=0.1)

#log.odds.shelf.1 = beta.hat[1,1] + beta.hat[1,2] * sugar + mean.fat + mean.sodium
#log.odds.shelf.3 = beta.hat[2,1] + beta.hat[2,2] * sugar + mean.fat + mean.sodium
#log.odds.shelf.4 = beta.hat[3,1] + beta.hat[3,2] * sugar + mean.fat + mean.sodium

#pi.hat.shelf.1 = exp(log.odds.shelf.1)/(1 + exp(log.odds.shelf.1) + exp(log.odds.shelf.3) + exp(log.odds.shelf.4))
#pi.hat.shelf.2 = 1/(1 + exp(log.odds.shelf.1) + exp(log.odds.shelf.3) + exp(log.odds.shelf.4))
#pi.hat.shelf.3 = exp(log.odds.shelf.3)/(1 + exp(log.odds.shelf.1) + exp(log.odds.shelf.3) + exp(log.odds.shelf.4))
#pi.hat.shelf.4 = exp(log.odds.shelf.4)/(1 + exp(log.odds.shelf.1) + exp(log.odds.shelf.3) + exp(log.odds.shelf.4))

#plot(x=sugar, y=pi.hat.shelf.1, col=1, lty=1, lwd=2, ylim=c(0,1), xlim=c(0,1),
#     ylab='estimated probability for a shelf', xlab='sugar per serving (after normalized)')
#lines(x=sugar, y=pi.hat.shelf.2, col=2, lty=2, lwd=2)
#lines(x=sugar, y=pi.hat.shelf.3, col=3, lty=3, lwd=2)
#lines(x=sugar, y=pi.hat.shelf.4, col=4, lty=4, lwd=2)
#legend('topleft', cex=0.8, inset=.05, title='Shelf Number', legend=seq(1,4), col=seq(1,4), lty=seq(1,4), lwd=rep(3,4))

# method 2: automatically provide values for sugar 
...
curve(expr = exp(beta.hat[2,1] + beta.hat[2,2]*x + mean.fat + mean.sodium)/(1 + exp(beta.hat[1,1] + beta.hat[1,2]*x + mean.fat + mean.sodium) + exp(beta.hat[2,1] + beta.hat[2,2]*x + mean.fat + mean.sodium) + exp(beta.hat[3,1] + beta.hat[3,2]*x + mean.fat + mean.sodium)),
  ...
  xlim = c(min(norm.d$sugar[norm.d$shelf == 3]), max(norm.d$sugar[norm.d$shelf == 1]))) 
...
data.frame(sugar, round(predict(object = m, newdata = data.frame(sugar = sugar, fat = mean.fat, sodium = mean.sodium), type = "probs"), 2))
```
...
curve(expr = exp(beta.hat[2,1] + beta.hat[2,3]*x + mean.sugar + mean.sodium)/(1 + exp(beta.hat[1,1] + beta.hat[1,3]*x + mean.sugar + mean.sodium) + exp(beta.hat[2,1] + beta.hat[2,3]*x + mean.sugar + mean.sodium) + exp(beta.hat[3,1] + beta.hat[3,3]*x + mean.sugar + mean.sodium)),
  col = 3, lty = 3, lwd = 2, n = 1000, add = TRUE,
  xlim = c(min(norm.d$fat[norm.d$shelf == 3]), max(norm.d$fat[norm.d$shelf == 1]))) 
# pi.hat.shelf.4
...
curve(expr = 1/(1 + exp(beta.hat[1,1] + beta.hat[1,4]*x + mean.sugar + mean.fat) + exp(beta.hat[2,1] + beta.hat[2,4]*x + mean.sugar + mean.fat) + exp(beta.hat[3,1] + beta.hat[3,4]*x + mean.sugar + mean.fat)), 
      ...
      panel.first = grid(col = "gray", lty = "dotted"))
# pi.hat.shelf.1
curve(expr = exp(beta.hat[1,1] + beta.hat[1,4]*x + mean.sugar + mean.fat)/(1 + exp(beta.hat[1,1] + beta.hat[1,3]*x + mean.sugar + mean.fat) + exp(beta.hat[2,1] + beta.hat[2,3]*x + mean.sugar + mean.fat) + exp(beta.hat[3,1] + beta.hat[3,3]*x + mean.sugar + mean.fat)),
  col = 1, lty = 1, lwd = 2, n = 1000, add = TRUE,
...
# pi.hat.shelf.3
curve(expr = exp(beta.hat[2,1] + beta.hat[2,3]*x + mean.sugar + mean.fat)/(1 + exp(beta.hat[1,1] + beta.hat[1,3]*x + mean.sugar + mean.fat) + exp(beta.hat[2,1] + beta.hat[2,3]*x + mean.sugar + mean.fat) + exp(beta.hat[3,1] + beta.hat[3,3]*x + mean.sugar + mean.fat)),
  col = 3, lty = 3, lwd = 2, n = 1000, add = TRUE,
  xlim = c(min(norm.d$sodium[norm.d$shelf == 3]), max(norm.d$sodium[norm.d$shelf == 1]))) 
# pi.hat.shelf.4
curve(expr = exp(beta.hat[3,1] + beta.hat[3,3]*x + mean.sugar + mean.fat)/(1 + exp(beta.hat[1,1] + beta.hat[1,3]*x + mean.sugar + mean.fat) + exp(beta.hat[2,1] + beta.hat[2,3]*x + mean.sugar + mean.fat) + exp(beta.hat[3,1] + beta.hat[3,3]*x + mean.sugar + mean.fat)),
  col = 4, lty = 4, lwd = 2, n = 1000, add = TRUE,
...
print('odd ratio for shelf 1 vs shelf 2:')
#round(exp(sd.d * beta.hat[1, 2:4]), 2)
round(1/exp(sd.d * beta.hat[1, 2:4]), 2)
round(data.frame(low=1/ci.or.shelf.1[,2], up=1/ci.or.shelf.1[,1]), 2)

print('odd ratio for shelf 3 vs shelf 2:')
#round(exp(sd.d * beta.hat[2, 2:4]), 2)
round(1/exp(sd.d * beta.hat[2, 2:4]), 2)
round(data.frame(low=1/ci.or.shelf.2[,2], up=1/ci.or.shelf.2[,1]), 2)

print('odd ratio for shelf 4 vs shelf 2:')
#round(exp(sd.d * beta.hat[3, 2:4]), 2)
